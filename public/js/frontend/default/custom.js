...
var getquotetdesign = { product:null, sides :null, sizes :null, num_shirts:0, num_colors:0};
var collapsemenu = true;
var currentmenu = '';

window.onload = function()
{
    if(parent.indesignstudio && document.getElementById('product_page') != undefined)
    {
        document.getElementById('product_page').style.marginTop = '0px';
    }
    /*else
    {
        document.getElementById('product_page').style.marginTop = '150px';
    }*/
    var today = new Date();
    ...
    $.ajax({
        type:'GET',
        dataType: 'json',
        url: window.location.origin + '/api/defaultproduct',
        success:function(product)
        {
            getquotetdesign.product = product;
            getquotetdesign.sides = product.sides;
            getquotetdesign.sizes = product.sizes;
        }
    });
...
function refreshcaptcha()
 {
    $.ajax({
        type:'GET',
        dataType: 'json',
        url: window.location.origin + '/refreshcaptcha',
 ...
 function setpricequoteproduct(idproduct)
 {
    $.ajax({
        type:'GET',
        dataType: 'json',
        url: window.location.origin + '/price-quote-selected-product/'+idproduct,
        success:function(data)
        {
            document.getElementById('main-get-quote').style.display =  'block';
            document.getElementById('product-list-get-quote').style.display =  'none';
            document.getElementById("product-color-nominimum").innerHTML =  "";
            document.getElementById("product-color-nominimum").innerHTML =  data.info;
            document.getElementById("product-color-minimum").innerHTML =  "";
            document.getElementById("product-color-minimum").innerHTML =  data.info2;
            document.getElementById("shirt-quantity").innerHTML =  "";
            document.getElementById("shirt-quantity").innerHTML =  data.sizes;
            ...
            document.getElementById("currentproductgetquote").innerHTML =  "";
            document.getElementById("currentproductgetquote").innerHTML =  data.currentproduct.name;
            document.getElementById("currentcolorname").innerHTML =  "";
            document.getElementById("currentcolorname").innerHTML =  data.currentproduct.color.name;
            getquotetdesign.product =  data.currentproduct;
            getquotetdesign.sides =  data.currentproduct.sides;
            getquotetdesign.sizes =  data.currentproduct.sizes;
        }
    });
 }

 function displayproductgetquote()
 {
    document.getElementById('main-get-quote').style.display =  'none';
    document.getElementById('product-list-get-quote').style.display =  'block';
    document.getElementById('listproduct').style.display = 'block';
    document.getElementById('singleproduct').style.display = 'none';
 }

function getquotesetsize(element)
{
    getquotetdesign.num_shirts = 0;
    for (var i = 0; i < getquotetdesign.product.sizes.length; i++)
    {
        if(getquotetdesign.product.sizes[i].name == element.id)
        {
            getquotetdesign.product.sizes[i].quantity =  Number(element.value);
            getquotetdesign.num_shirts +=  Number(element.value);
            //break;
        }
        else
        {
            getquotetdesign.num_shirts += getquotetdesign.product.sizes[i].quantity;
        }
    }
    calc_get_quote();
}
...
function getquotesetside(element)
{
    var colors = 0;
    for (var i = 0; i < getquotetdesign.product.sides.length; i++)
    {
        if(getquotetdesign.product.sides[i].slug == element.id)
        {
            getquotetdesign.product.sides[i].quantity =  Number(element.value);
            colors = Number(colors + element.value);
        }
        else
        {
            if(getquotetdesign.product.sides[i].quantity > 0)
            {
                colors = Number(colors + getquotetdesign.product.sides[i].quantity);
            }
            else
            {
                getquotetdesign.product.sides[i].quantity =  0;
            }
        }
          getquotetdesign.num_colors = colors;
    }
    calc_get_quote();
}
...
function setactivecolor(element)
{
    ...
    for (i = 0; i < circlescolors.length; i++)
    {
       if(circlescolors[i] == element)
            element.className = element.className.replace("block ","block animate " );
        else
            circlescolors[i].className =  circlescolors[i].className.replace("block animate " ,"block ");
    }
}


function calc_get_quote()
{
    if(getquotetdesign.product != null && getquotetdesign.num_colors > 0 && getquotetdesign.num_shirts > 0)
    {
        $.ajax({
            type:'POST',
            dataType: 'json',
            data: {
                getquotetdesign: JSON.parse(JSON.stringify(getquotetdesign)),
            },
            url: window.location.origin + '/api/calcpricequote',
            success:function(data)
            {
                var total = data.price;
                document.getElementById('itemPrice').innerHTML =  total == 0 ? 0 : (total / getquotetdesign.num_shirts).toFixed(2);
                document.getElementById('itemNumOfShirt').innerHTML = getquotetdesign.num_shirts;
                document.getElementById('itemTotal').innerHTML = total;

                $('#select-product-tab').removeClass('active');
                $('#select-product').removeClass('active  show');
                $('#select-product').attr('aria-expanded',false);

                $('#product-color-tab').removeClass('active');
                $('#product-color').removeClass('active  show');
                $('#product-color').attr('aria-expanded',false);

                $('#number-color-tab').removeClass('active');
                $('#number-color').removeClass('active  show');
                $('#number-color').attr('aria-expanded',false);

                $('#shirt-quantity-tab').removeClass('active');
                $('#shirt-quantity').removeClass('active  show');
                $('#shirt-quantity').attr('aria-expanded',false);

                $('#get-quote-tab').addClass('active');
                $('#get-quote').addClass('active  show');
 ...
 function setpricequoteproductcolor(name,id)
 {
    document.getElementById("currentproductgetquote").innerHTML =  "";
    document.getElementById("currentproductgetquote").innerHTML =  name;
    $.ajax({
        type:'GET',
        dataType: 'json',
        data: {id: id},
        url: window.location.origin + '/api/productbyid',
        success:function(product)
        {
            getColorProduct(product.color_id);
            getquotetdesign.product = product;
            getquotetdesign.sides = product.sides;
            getquotetdesign.sizes = product.sizes;
        }
...
function getColorProduct(color_id) {
    $.ajax({
        type:'GET',
        dataType: 'json',
        url: window.location.origin + '/api/getColorById/'+color_id,
        success:function(response)
        {
            $('#currentcolorname').html(response[0].name)
        }
    });     
}
 function setpricequotenumbercolor()
 {
    ...
    $('#get-quote-tab').removeClass('active');
    $('#get-quote').removeClass('active  show');
    $('#get-quote').attr('aria-expanded',false);
    ...
    $('#number-color').addClass('active  show');
 ...
 {
    pricequote_amnt = 0;
    for (var i = 0; i < product.parent.sizes.length; i++)
    {
        if(product.parent.sizes[i].slug == element.id)
        {
            product.parent.sizes[i].quantity =  Number(element.value);
            pricequote_amnt +=  Number(element.value);
            //break;
        }
        else
        {
            pricequote_amnt += product.parent.sizes[i].quantity;
        }
    }
 }
 ...
 function setpricequoteestimation()
 {
    $.ajax({
        ...
        dataType: 'json',
        data: {
            pricequote_product: pricequote_product,
...
function display_child_product_image(url_imag )
{ 
  var fullurl =  window.location.origin + '/img/product/' + url_imag;
  document.getElementById('default-image').src = fullurl;
  document.getElementById('default-image-thumb').src = fullurl; 
  document.getElementsByClassName('lSPager lSGallery')[0].firstChild.firstChild.firstChild.src = fullurl;
...
async function loadproductquote()
{
    var productpricequotelist = document.getElementsByClassName('pink_color');
    for (let index = 0; index < productpricequotelist.length; index++) {
        const element = productpricequotelist[index];
        getdataproductprice(element);
    }
}

async  function getdataproductprice(value){
     $.ajax({
            type:'POST',
            dataType: 'json',
            data: {
                idproduct: value.id.replace('productpricequote_',''),
                price_quote_quantity: document.getElementById("price_quote_quantity").value,
                price_quote_front_color: document.getElementById("price_quote_front_color").value,
                price_quote_back_color: document.getElementById("price_quote_back_color").value
            },
            url: window.location.origin + '/api/productpricequote',
            success:function(data)
            {
               document.getElementById(value.id).innerHTML = "$ " + data + " each -";
            }
        });
}
  ...
  function validate_signup()
    {
        ...
        var special_offers = document.getElementById('special_offers');

       if(firstName.value != "" && lastName.value != "" && user_name.value != "" && pass_word.value != "" && confirm_password.value != "" && email_address.value != "")
       {
       ...
       else
       {
          ...
          {
            user_name.focus();
            user_name.style.borderColor = "transparent";
          }

          if (pass_word.value == "")
          {
            pass_word.focus();
            pass_word.style.borderColor = "red";
          }
          else
          {
            pass_word.focus();
            pass_word.style.borderColor = "transparent";
          }
          ...
          {
            confirm_password.focus();
            confirm_password.style.borderColor = "transparent";
          }
          ...
          {
            email_address.focus();
            email_address.style.borderColor = "transparent";
          }
       }
    }
...
function validatedate(){
 ...
 {
    document.getElementById('go_designstudio').href= window.location.origin+"/designstudio/p/"+childproductid; 
    document.getElementById('single-product-selected-color').className = 'block  colour-colorDisplay color-'+colorid; 
    document.getElementById('single-product-selected-name').innerHTML = colorname;    
 }

 function filterall(element,from)
 {
    var newurlfabric = newurlbrand = newurlcategory = newurl = '';  
    if(element.id == 'FilterFabricViewAll' && element.checked)
    { 
        var filtersfabric = document.getElementsByClassName('FilterFabric');
        for (let index = 0; index < filtersfabric.length; index++) 
        {
            const filtertype = filtersfabric[index];
            filtertype.checked = false;
        }
        newurlfabric = '&fabric=ViewAll';
    }
    else if(element.id == 'FilterBrandViewAll' && element.checked)
    { 
         var filtersbrand = document.getElementsByClassName('FilterBrand');
         for (let index = 0; index < filtersbrand.length; index++) 
         {
             const filtertype = filtersbrand[index];
             filtertype.checked = false;
         }
         newurlbrand = '&brand=ViewAll';
     }
     else if(element.id.indexOf('-FilterCategory') >= 0 && element.checked)
    { 
         var filterscategory = document.getElementsByClassName(element.id);
         for (let index = 0; index < filterscategory.length; index++) 
         {
             const filtertype = filterscategory[index];
             filtertype.checked = false;
         }
         newurlcategory = '&category='+element.id.replace('-FilterCategory','')+'/ViewAll';
     }
     ...
     {
        const urlfabricparameters = findGetParameter('fabric');   
        if(urlfabricparameters != null)
        {        
            newurl += '&fabric=' + urlfabricparameters;
        }
        else
        {
            newurl += '';
     ...
     {
        const urlbrandparameters = findGetParameter('brand');   
        if(urlbrandparameters != null)
        {        
            newurl += '&brand=' + urlbrandparameters;
        }
        else
        {
            newurl += '';
     ...
     if(newurlcategory!= '')
     {
        newurl += newurlcategory;
     }
     else
     {
        const urlcategoryparameters = findGetParameter('category');   
        if(urlcategoryparameters != null)
        {        
            newurl += '&category=' + urlcategoryparameters;
        }
        else
        {
            newurl += '';
    ...
    window.location.href = window.location.origin + from + newurl;
 }

 function removefilterproduct(type,value,from)
 {
    var newurl = '';   

    if(type != 'all')
    {
        var urlbrandparameters = '';
        var urlfabricparameters = '';
        var urlcategoryparameters = '';
        if(type == 'brand')
        {
            const brand = findGetParameter('brand');   
            if(brand != null)
            {        
                elements = brand.split("_");
                ...
                {
                    if(brand.indexOf(value+'_') >= 0)
                    {
                        
                        urlbrandparameters = brand.replace(value + '_', "");
                    }
                    
                    else if(brand.indexOf('_'+value) >= 0)
                    {
                        urlbrandparameters = brand.replace('_' + value, "");
                    }        
                    else
                    {
                        urlbrandparameters = brand.replace(value , "");
                    }    
                }
            }
            if(urlbrandparameters != '')
            {
                newurl = 'brand=' + urlbrandparameters;
            } 
        } 
        ...
        {
            urlbrandparameters = findGetParameter('brand'); 
            if(urlbrandparameters != null)
            {
                newurl = 'brand=' + urlbrandparameters;
            } 
        ...
        if(type == 'fabric')
        {        
            const fabric = findGetParameter('fabric');        
            if(fabric != null )
            {        
                elements = fabric.split("_");
                if(elements.length == 1)
                {
                    urlfabricparameters = '';
                }
                ...
                {
                    if(fabric.indexOf(value+'_') >= 0)
                    {
                        urlfabricparameters = fabric.replace(value + '_', "");
                    }
                    
                    else if(fabric.indexOf('_'+value) >= 0)
                    {
                        urlfabricparameters = fabric.replace('_' + value, "");
                    }        
                    else
                    {
                        urlfabricparameters = fabric.replace(value , "");
                    }      
                    console.log("urlfabricparameters: " + urlfabricparameters);
                }
            } 

            if(urlfabricparameters != '')
            {
                if(newurl != '')
                {
                    newurl += '&fabric=' + urlfabricparameters;
        ...
        {
            urlfabricparameters = findGetParameter('fabric'); 
            if(urlfabricparameters != null)
            {
                if(newurl != '')
                {
        ...
        if(type == 'category')
        {        
            const category = findGetParameter('category');    
            if(category != null )
            {        
                elements = category.split("_");
                if(elements.length == 1)
                {
                    urlcategoryparameters = '';
                }
                else
                {
                    if(category.indexOf(value+'_') >= 0)
                    {
                        urlcategoryparameters = category.replace(value + '_', "");
                    }
                    
                    else if(category.indexOf('_'+value) >= 0)
                    {
                        urlcategoryparameters = category.replace('_' + value, "");
                    }        
                    else
                    {
                        urlcategoryparameters = category.replace(value , "");
                    }      
                }
            } 

            if(urlcategoryparameters != '')
            {
                if(newurl != '')
                {
                    newurl += '&category=' + urlcategoryparameters;
        ...
        {
            urlcategoryparameters = findGetParameter('category');
            if(urlcategoryparameters != null)
            {
                if(newurl != '')
                {
...
Date.prototype.format = function (mask, utc) {
    ...
    function colapsemenu(element,menu) 
    {
...
function ShowHideAnswers(elements)
        {
        ...
        function ShowHideOnlyAnswers(elements)
        {
            var answers =  document.getElementsByClassName("answers");
            for (var i = 0; i< answers.length; i++)
            {
             ...
             answers =  document.getElementById("answers"+elements);
        ...
        $(window).resize(function() {
            ...
            $is_menu_opened = $('.mobile_bar.activemobile_bar').length;
            if($is_menu_opened && $(this).width() > 990) {
