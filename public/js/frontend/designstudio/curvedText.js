(function (global){
	...
	_dimensionAffectingProps['arcangle']=true;
			...
			if(this.letters){
				...
				if(this.effect==='curved'){
					for(var i=0, len=this.text.length; i<len; i++)
					{
						...
						arcLength += (this.letters.item(i).width + space);
						this.radius = (arcLength*360)/ (2*Math.PI*this.arcangle);
					}
					textWidth-=space;
				...
				//this.radius = this.letters.length + space / Math.PI;
...
//				console.log('End rendering')
		...
		/*_polarCoords(arc, letters) {
                ...
                if (circle.radius < 0) {
                    x = (circle.radius - letters.base * 2 / 3) * cos + circle.center[0], y = (circle.radius - letters.base * 2 / 3) * sin + circle.center[1];
                }
                return [x, y, angle];
        },*/
...
//				}
		 ...
		 */
		toString: function (){
			return '#<fabric.CurvedText ('+this.complexity()+'): { "text": "'+this.text+'", "fontFamily": "'+this.fontFamily+'", "radius": "'+this.radius+'", "arcangle": "'+this.arcangle+'", "spacing": "'+this.spacing+'", "reverse": "'+this.reverse+'" }>';
		},
		/* _TO_SVG_START_ */
		/**
