  ...
  <section>
                    ...
                    <div id="all-colors" class="collapse show" aria-labelledby="collapseColor" data-parent="#accordioncolors">  
                        <div v-for="(familycolor, indexcolor) in familycolors" :key="familycolor.id">
                            <label class="radio-container" style="float: left; width: 20px;"><pre></pre>
                                <input class="FilterColor" type="checkbox" v-bind:id="familycolor.name" v-model="filtersSelected" :value="'Color:'+familycolor.name" v-on:change="getfilteredData">
                            ...
                            <label class="radio-container" style="float: left; width: 20px;" v-if="indexcolor == familycolors.length-1"><pre></pre>
                                <input class="FilterColor" type="checkbox" v-bind:id="'View All'"  v-model="filtersSelected" value="Color:View All" v-on:change="getfilteredData">
    ...
    <div class="col-md-9 col-sm-12 product_responsive" id="listproduct">
        ...
        <div class="row" v-show="filtersSelected.length > 0">
                ...
                <h5> Filters: </h5> 
                    <button type="button" class="product-filter" v-show="lessfilters == true"  v-for="(filter) in firstfiltersSelected" :key="'less'+filter">{{filter}} <span  class="close" style="margin-left: 3px; margin-top: -6px;"  v-on:click="removeFilter(filter)">&times;</span></button>
                    <button type="button" class="product-filter" v-show="lessfilters == false"  v-for="(filter) in filtersSelected" :key="'more'+filter">{{filter}} <span  class="close" style="margin-left: 3px; margin-top: -6px;"  v-on:click="removeFilter(filter)">&times;</span></button>
                    <button type="button" class="product-filter" v-show="lessfilters == true && (filtersSelected.length-firstfiltersSelected.length) > 0" v-on:click="showfilters('more')">+{{(filtersSelected.length-firstfiltersSelected.length)}}</button>
                    <button type="button" class="product-filter" v-show="lessfilters == false"  v-on:click="showfilters('less')">Show Less</button>
        ...
        <div class="row" v-show="filtersSelected.length > 0">     
            <div class="col-sm-4" style="margin-top: 20px; margin-bottom: 10px;"  v-for="product in filterProducts" :key="product.id">        
                <div class="product_printing" style="margin-bottom: 10px;">
                    <a v-bind:href=" path + '/product/'+ product.id"> 
                ...
                <center>
                    <div style="padding-top: 10px; margin-right:0px;">
                        <div  v-for="productcolor in product.products" :key="productcolor.id" :class="'block  colour-colorDisplay  color-'+ productcolor.color_id" v-if="selectedFilters.indexOf('Color:'+productcolor.color.colorfamily) >= 0 || selectedFilters.indexOf('Color:View All') >= 0" v-bind:id="'productcolor-'+ productcolor.id"  v-on:mouseover="displayChildImage(productcolor);">
                        </div>
                    </div>                    
                </center>
            </div>
...
export default {
    ...
    computed:{
        selectedFilters: function() {   
            ...
            { 
                this.firstfiltersSelected = this.filtersSelected.slice(0, 4);
                this.filtersSelected.forEach(element => {
                    filters.push(element);
                });
            }
            else if(this.filtersSelected)
            {
                filters.push(this.filtersSelected);
                this.firstfiltersSelected.push(this.filtersSelected);
            }
            ...
            if (this.selectedFilters.length > 0)
            {
                this.filterProducts.forEach(element =>
                {
                    element.products.forEach(child=>{
                        ...
                        {  
                            let url_imag =  window.location.origin + '/vivid_test/img/product/' + child.image;  element.image = child.image;
                            if(!hascolor)
                            {
                                //counthascolor ++;
                                hascolor = true;
