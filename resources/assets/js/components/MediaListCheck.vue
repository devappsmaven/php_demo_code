...
<template>
    <div class="row">
        <div style="position:relative; display:inline" v-for="media in medias.data" :key="media.id" v-bind:style="'width: 110px; height: 110px;'">
            <label class="image-checkbox"   v-bind:name="slug + 'media[]'" v-bind:id="'label'+media.id"  v-bind:value=" path + '/img/' + media.url + '/' + media.file" v-on:click="setchecked($event);">
                <i class="fa fa-check hidden"></i>
                <img class="img-responsive" v-bind:id="'img'+media.id"  v-lazy=" path + '/img/' + media.url + '/' + media.file"  style="width:100px; max-width:100px; max-height: 100px;" />
                <input type="checkbox" v-bind:name="slug + 'media[]'" v-bind:id="'check'+media.id" v-bind:value=" path + '/img/' + media.url + '/' + media.file"  v-on:change="previewmedia($event.target);"/>
            </label>
        </div>
         <br>
...
Vue.use(VueLazyload, {
  ...
  loading: window.location.origin + '/img/others/loading.gif',
  attempt: 1,
    ...
    methods:{
        ...
        setchecked(e)
        {
            if(e.target != undefined && e.target.type != "checkbox")
            {
                ...
                if(document.getElementById(e.target.id.replace('img','label')).className.indexOf("image-checkbox-checked") == -1)
                {
                    $("#"+ e.target.id.replace('img','label')).addClass('image-checkbox-checked');
                    setupImage(e.target.src,displayadditionalimages);
                }
                else
                {
                    $("#"+e.target.id.replace('img','label')).removeClass('image-checkbox-checked');
                    removeimage(e.target.src);
                }
            }
            e.preventDefault();
        },
