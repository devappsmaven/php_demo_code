...
@section('title','Manage Clipart Categories Embroidery')
@section('pagename','Manage Clipart Categories Embroidery')
@section('menuname','Manage Clipart Categories Embroidery')
@section('content')
@include('common.alerts')
<div class="box">
    <div class="box-header">
        <h3 class="box-title">Clipart Categories Embroidery List</h3>
        @can('clipartcategoriesembroidery.create')
        <a class="btn-primary  btn-sm pull-right add-record" data-toggle="modal" data-target="#custom-width-modal2" data-url="{!! URL::route('clipartcategoriesembroidery.store') !!}" data-token="{{ csrf_token() }}">
            Add Clipart Categories
        </a>
    @endcan
    @can('clipartsembroidery.addclipart')
        <a class="btn-success  btn-sm pull-right  add-clipart" data-toggle="modal" data-target="#custom-width-modal3" data-url="{!! URL::route('clipartsembroidery.addclipart') !!}" style=" cursor:pointer;">
            Add Clipart Embroidery
        </a>
    @endcan
    </div>
    <div class="box-body">
        <div class="timeline-body" style="display:none;">
            @foreach ($clipartcategoriesembroidery as $clipartcategoryembroidery)
                @can('clipartsubcategoriesembroidery.create')
                    <a href="{{route('clipartsubcategoriesembroidery.index', $clipartcategoryembroidery->id)}}">
                        <img src="{{asset('img/clipart-embroidery')}}{{'/'}}{{$clipartcategoryembroidery->slug}}{{'/'}}{{$clipartcategoryembroidery->filename}}" alt="{{ $clipartcategoryembroidery->slug }}" style="max-width:74px !important; max-height: 74px !important;">
                    </a>
                @endcan
            @endforeach
        </div>
                ...
                <div class="col-sm-2">
                    <div class="box box-primary widget-user">
                        <div class="widget-user-header">
                            <h4>{{ $clipartcategoryembroidery->title }}</h4>
                        </div>
                        <div class="widget-user-image">
                            @can('clipartsubcategoriesembroidery.index','clipartsubcategoriesembroidery.create','clipartsubcategoriesembroidery.edit','clipartsubcategoriesembroidery.show')
                                <a href="{{route('clipartsubcategoriesembroidery.index', $clipartcategoryembroidery->id)}}">
                                    <img src="{{asset('img/clipart-embroidery')}}{{'/'}}{{$clipartcategoryembroidery->slug}}{{'/'}}{{$clipartcategoryembroidery->filename}}" alt="{{ $clipartcategoryembroidery->slug }}"  style="max-width:74px !important; max-height: 74px !important;">
                                </a>
                            @endcan
                        ...
                        <div class="box-footer">
                            ...
                            <div class="col-sm-4">
                                <div class="description-block">
                                    <div class="btn-group">
                                        @can('clipartsubcategoriesembroidery.create')
                                            <a href="{{route('clipartcategoriesembroidery.show', $clipartcategoryembroidery->id)}}" class="btn btn-sm btn-info">View</a>
                                        @endcan
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="description-block">
                                    @can('clipartcategoriesembroidery.edit')
                                        <a href="{{route('clipartcategoriesembroidery.edit', $clipartcategoryembroidery->id)}}" class="btn btn-sm btn-warning">Edit</a>
                                    @endcan
                                </div>
                            </div>
                            <div class="col-sm-4">
                                <div class="description-block">
                                    <div class="btn-group">
                                        @can('clipartcategoriesembroidery.destroy')
                                            <a class="btn btn-danger btn-sm waves-effect waves-light remove-record" data-toggle="modal" data-target="#custom-width-modal" data-url="{!! URL::route('clipartcategoriesembroidery.destroy', $clipartcategoryembroidery->id) !!}" data-id="{{$clipartcategoryembroidery->id}}" data-token="{{ csrf_token() }}">Remove</a>
                                        @endcan
                                    </div>
                                </div>
                            </div>
        ...
        <div id="custom-width-modal2" class="modal modal-default fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
            <div class="modal-dialog" style="width:55%;">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        <h4 class="modal-title" id="custom-width-modalLabel">Create Clipart Category Embroidery</h4>
                    </div>
                    <div class="modal-body">
                        <div class="box-body">
        ...
        <div id="custom-width-modal3" class="modal modal-default fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
            <div class="modal-dialog" style="width:55%;">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        <h4 class="modal-title" id="custom-width-modalLabel">Create Clipart Embroidery</h4>
                    </div>
                    <div class="modal-body">
                        <div class="box-body">
                            ...
                            <selectembroiderycomponent></selectembroiderycomponent>
                            <br>
                                ...
                                <div class="box-body" style="display: none;">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" multiple="multiple" data-placeholder="Select a category" style="width: 100%;" name="categories[]">
                                            @foreach ($clipartcategoriesembroidery as $clipartcategoryembroidery)
                                                <option value="{{ $clipartcategoryembroidery->id }}" disabled>{{ $clipartcategoryembroidery->slug }}</option>
                                                @foreach ($clipartsubcategoriesembroidery as $clipartsubcategoryembroidery)
                                                    @if($clipartcategoryembroidery->id == $clipartsubcategoryembroidery->category_embroidery_id)
                                                        <option value="{{ $clipartsubcategoryembroidery->category_embroidery_id }}{{ '_' }}{{ $clipartsubcategoryembroidery->id }}">&nbsp;&nbsp;&nbsp;{{ $clipartsubcategoryembroidery->slug }}</option>  
                                                    @endif
                                                @endforeach                                               
                                            @endforeach
                                        </select> 
