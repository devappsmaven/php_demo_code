...
@section('title','Edit Shirt Color')
@section('pagename','Edit Shirt Color')
@section('menuname','Edit Shirt Color')
@section('content')
...
<div class="row" style="margin-top:50px;">    
    ...
    <div class="col-md-6"> 
        ...
        <form role="form" method="POST" action="/admin/shirtcolors/{{$shirtcolor->id}}" enctype="multipart/form-data" class="colors_form" novalidate="novalidate">
            {{csrf_field()}}
            {{method_field('PUT')}}
                <div class="box-body">
                        <div class="form-group{{ $errors->has('order') ? ' has-error' : '' }} has-feedback">
                    <label for="order">Order</label>
                    <input type="number" class="form-control" name="order" value="{{$shirtcolor->order}}">
                    @if ($errors->has('order'))
                    ...
                    <select class="form-control" name="colorfamily">
                        @foreach ($colorfamily as $color)
                            @if($color->status == 'Enable')
                                @if($color->name == $shirtcolor->colorfamily)
                ...
                <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }} has-feedback">
                    ...
                    <input type="text" class="form-control" name="name" placeholder="Enter Name" value="{{$shirtcolor->name}}">
                    @if ($errors->has('name'))
                    <span class="invalid-feedback" role="alert">
                ...
                <div class="form-group{{ $errors->has('hex') ? ' has-error' : '' }} has-feedback">
                    ...
                    <div class="input-group my-colorpicker2">
                        <input type="text" class="form-control" name="hex" value="{{$shirtcolor->hex}}">
                ...
                <div class="form-group{{ $errors->has('file') ? ' has-error' : '' }} has-feedback">
                    ...
                    <input type="file" class="form-control" name="file" value="{{$shirtcolor->file}}">
                    @if ($errors->has('file'))
                        <span class="invalid-feedback" role="alert">
            ...
            <div class="form-group{{ $errors->has('status') ? ' has-error' : '' }} has-feedback">
                <label for="status">Status</label>
                    <select class="form-control" name="status">
                        @if($shirtcolor->status == 'Enable')
                           <option selected>Enable</option>
                           <option>Disable</option>
                        @endif
                        @if($shirtcolor->status == 'Disable')
