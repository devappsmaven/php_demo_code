...
@section('title','Manage Clipart Embroidery')
@section('pagename','Manage Clipart Embroidery')
@section('menuname','Manage Clipart Embroidery')
@section('content')
@include('common.alerts')
<div class="box">
    <div class="box-header">
        <h3 class="box-title">Clipart Embroidery List</h3>
        @can('clipartsembroidery.removeall')
            <a class="btn-danger btn-sm pull-right remove-all" id="removeall"  data-toggle="modal" data-target="#custom-width-modal4" style="display:none; margin-left: 10px; cursor:pointer;" data-target="#custom-width-modal" data-url="{!! URL::route('clipartsembroidery.removeall') !!}" data-token="{{ csrf_token() }}">Delete All Selected</a>
        @endcan
        ...
        @can('clipartsembroidery.edit')
            <a class="btn-warning btn-sm pull-right copymove-record" id="copymove"  data-toggle="modal" data-target="#custom-width-modal3" style="display:none; margin-left: 10px; cursor:pointer;" data-target="#custom-width-modal" data-url="{!! URL::route('clipartsembroidery.copymove') !!}" data-token="{{ csrf_token() }}">Copy/Move</a>
        @endcan
        @can('clipartsubcategoriesembroidery.index')
            <a href="{{route('clipartsubcategoriesembroidery.index', $clipartcategoryembroidery->id)}}" class="btn-info btn-sm pull-right" style="margin-left: 10px; cursor:pointer;">Back to SubCategories</a>
        @endcan
        @can('clipartsembroidery.create')
            <a class="btn-primary  btn-sm pull-right  add-record" data-toggle="modal" data-target="#custom-width-modal2" data-url="{!! URL::route('clipartsembroidery.store') !!}" style=" cursor:pointer;">
                Add Clipart
            </a>
        @endcan
    </div>
    <div class="box-body">
        <div class="row">
            @foreach ($clipartsembroidery as $clipartembroidery)
                <div class="col-sm-2">
                    <input type="checkbox" id="{{ $clipartembroidery->id }}" class="clipart-check">
                    <div class="box box-widget widget-user">
                        <div class="widget-user-header">
                            <h5>{{ $clipartembroidery->title }}</h5>
                            <h5 class="widget-user-desc"><a href="{{route('clipartcategoriesembroidery.index', $clipartembroidery->categoryembroidery->id)}}">{{ $clipartembroidery->categoryembroidery->title }}</a> / <a href="{{route('clipartsubcategoriesembroidery.index', [$clipartembroidery->categoryembroidery->id, $clipartembroidery->subcategoryembroidery->id])}}">{{ $clipartembroidery->subcategoryembroidery->title }}</a></h5>
                        </div>
                        <div class="widget-user-image">
                            <img  src="{{asset('img/clipart-embroidery')}}{{'/'}}{{$clipartembroidery->categoryembroidery->slug}}{{'/'}}{{$clipartembroidery->subcategoryembroidery->slug}}{{'/'}}{{$clipartembroidery->filename}}" alt="{{ $clipartembroidery->title }}" style="padding: 1vw;max-width: 110px; max-height: 110px">
                        </div>
                            ...
                            <div class="row">
                                <div class="col-sm-4 border-right">
                                    <div class="description-block">
                                        <div class="btn-group">
                                            @can('clipartsembroidery.show')
                                                <a href="{{route('clipartsembroidery.show', $clipartembroidery->id)}}" class="btn btn-sm btn-info">View</a>
                                            @endcan
                                        </div>
                                    </div>
                                </div>
    ...
    <div id="custom-width-modal2" class="modal modal-default fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" style="width:55%;">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <input type="hidden" name="category_id" value={{ $clipartcategoryembroidery->id }}>
                    <input type="hidden" name="subcategory_id" value={{ $clipartsubcategoryembroidery->id }}>
                    <div class="box-body">
                        <div class="form-group">
                            <label for="title">Category Name</label>
                            <input type="text" class="form-control" name="category"  value="{{ $clipartcategoryembroidery->title}}" readonly>
                        </div>
                        <div class="form-group">
                            <label for="title">SubCategory Name</label>
                            <input type="text" class="form-control" name="subcategory"  value="{{ $clipartsubcategoryembroidery->title}}" readonly>
                        </div>
