...
<div class="box">
    <div class="box-header">
        ...
        @can('templates.categories')
            <a href="{{route('templates.createcategories')}}" class="btn-primary  btn-sm pull-right">
                Add Categories
            </a>
        @endcan
    </div><!-- /.box-header -->
        ...
        <tbody>
            @foreach ($categories as $category)
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            @can('templates.showcategories')
                                <a href="{{route('templates.showcategories', $category->id)}}" class="btn btn-sm btn-info">View</a>
                            @endcan
                        </div>
                        <div class="btn-group">
                            @can('templates.editcategories')
                                <a href="{{route('templates.editcategories', $category->id)}}" class="btn btn-sm btn-warning">Edit</a>
                            @endcan
                        </div>
                        <div class="btn-group">
                            @can('templates.destroycategories')
                                <a class="btn btn-danger btn-sm waves-effect waves-light remove-record" data-toggle="modal" data-target="#custom-width-modal" data-url="{!! URL::route('templates.destroycategories', $category->id) !!}" data-id="{{$category->id}}" data-token="{{ csrf_token() }}">Remove</a>
                            @endcan
                        </div>
                    </td>
                </tr>
