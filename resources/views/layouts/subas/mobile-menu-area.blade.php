						...
						@if($category->subcategories->count() > 0)
							<ul class="nav-items nav-expand-content">
								@foreach ($category->subcategories as $subcategory)
									<li class="nav-item">
										<a class="nav-link" href="{{route('product.category', [$category->slug,$subcategory->slug])}}">
											{{$subcategory->name}}
										</a>
									</li>	
								@endforeach
			...
			<li class="nav-item {{$category->subcategories->count() > 0 ? 'nav-expand' : '' }}"><a class="nav-link {{$category->subcategories->count() > 0 ? 'nav-expand-link' : '' }}" href="#">{{$category->name}}</a>
						@if($category->subcategories->count() > 0)
							<ul class="nav-items nav-expand-content">
								@foreach ($category->subcategories as $subcategory)
									<li class="nav-item">
										<a class="nav-link" href="{{route('template-designs.category', [$category->slug,$subcategory->slug])}}">
											{{$subcategory->name}}
										</a>
									</li>	
								@endforeach
