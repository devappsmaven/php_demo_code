...
@section('title','Manage Template Embroidery')
@section('pagename','Manage Template Embroidery')
@section('menuname','Manage Template Embroidery')
@section('content')
@include('common.alerts')
<div class="box">
    <div class="box-header">
        <h3 class="box-title">Template Embroidery List</h3>
        @can('templateembroidery.removeall')
            <a class="btn-danger btn-sm pull-right remove-all" id="removeall"  data-toggle="modal" data-target="#custom-width-modal4" style="display:none; margin-left: 10px; cursor:pointer;" data-target="#custom-width-modal" data-url="{!! URL::route('templateembroidery.removeall') !!}" data-token="{{ csrf_token() }}">Delete All Selected</a>
        @endcan
        ...
        @can('templateembroidery.edit')
            <a class="btn-warning btn-sm pull-right copymove-record" id="copymove"  data-toggle="modal" data-target="#custom-width-modal3" style="display:none; margin-left: 10px; cursor:pointer;" data-target="#custom-width-modal" data-url="{!! URL::route('templateembroidery.copymove') !!}" data-token="{{ csrf_token() }}">Copy/Move</a>
        @endcan
        @can('templatesubcategoriesembroidery.index')
            <a href="{{route('templatesubcategoriesembroidery.index', $templatecategoryembroidery->id)}}" class="btn-info btn-sm pull-right" style="margin-left: 10px; cursor:pointer;">Back to SubCategories</a>
        @endcan
        @can('templateembroidery.create')
            <a class="btn-primary  btn-sm pull-right  add-record" data-toggle="modal" data-target="#custom-width-modal2" data-url="{!! URL::route('templateembroidery.store') !!}" style=" cursor:pointer;">
                Add Template
            </a>
        @endcan
    </div>
    <div class="box-body">
        <div class="row">
            @foreach ($templatesembroidery as $templateembroidery)
                <div class="col-sm-2">
                    <input type="checkbox" id="{{ $templateembroidery->id }}" class="template-check">
                    <div class="box box-widget widget-user">
                        <div class="widget-user-header">
                            <h5>{{ $templateembroidery->title }}</h5>
                            <h5 class="widget-user-desc"><a href="{{route('templatecategoriesembroidery.index', $templateembroidery->categoryembroidery->id)}}">{{ $templateembroidery->categoryembroidery->title }}</a> / <a href="{{route('templatesubcategoriesembroidery.index', [$templateembroidery->categoryembroidery->id, $templateembroidery->subcategoryembroidery->id])}}">{{ $templateembroidery->subcategoryembroidery->title }}</a></h5>
                        </div>
                        <div class="widget-user-image">
                            <img  src="{{asset('img/template-embroidery')}}{{'/'}}{{$templateembroidery->categoryembroidery->slug}}{{'/'}}{{$templateembroidery->subcategoryembroidery->slug}}{{'/'}}{{$templateembroidery->filename}}" alt="{{ $templateembroidery->title }}" style="padding: 1vw;max-width: 110px; max-height: 110px">
                        </div>
                            ...
                            <div class="row">
                                <div class="col-sm-4 border-right">
                                    <div class="description-block">
                                        <div class="btn-group">
                                            @can('templateembroidery.show')
                                                <a href="{{route('templateembroidery.show', $templateembroidery->id)}}" class="btn btn-sm btn-info">View</a>
                                            @endcan
                                        </div>
                                    </div>
                                </div>
...
<form method="POST" class="remove-all-model">
    {{csrf_field()}}
    <input id="templatelist" type="hidden">
        ...
        <div class="modal-dialog" style="width:55%;">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4>You Want You Sure Delete The Selected Templates?</h4>
                </div>
    ...
    <div id="custom-width-modal3" class="modal modal-default fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" style="width:55%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <h4 class="modal-title" id="custom-width-modalLabel">Copy/Move Selected Templates</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-sm-6 ">
    ...
    <div id="custom-width-modal2" class="modal modal-default fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" style="width:55%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <h4 class="modal-title" id="custom-width-modalLabel">Create Templates</h4>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="category_id" value={{ $templatecategoryembroidery->id }}>
                    <input type="hidden" name="subcategory_id" value={{ $templatesubcategoryembroidery->id }}>
                    <div class="box-body">
                        <div class="form-group">
                            <label for="title">Category Name</label>
                            <input type="text" class="form-control" name="category"  value="{{ $templatecategoryembroidery->title}}" readonly>
                        </div>
                        <div class="form-group">
                            <label for="title">SubCategory Name</label>
                            <input type="text" class="form-control" name="subcategory"  value="{{ $templatesubcategoryembroidery->title}}" readonly>
                        </div>
                        ...
                        <div class="form-group{{ $errors->has('editable') ? ' has-error' : '' }} has-feedback">
                            ...
                            <div class="input-group">                                
                                ...
                                <input type="text" class="form-control" name="editabletext" id="editabletext" value="Non Editable" >
                            </div>     
                        </div>              
