...
@section('title','Open Cart Sended')
@section('pagename','Open Cart Sended')
@section('menuname','Open Cart Sended')
@section('content')
@guest
...
<div class="row">
    <div class="col-md-6">
        <div class="box box-primary">
            <form role="form" method="POST" action="{{route('admin.sendopencartemail')}}" enctype="multipart/form-data">
                {{csrf_field()}}
                                ...
                                <div class="tab-content">
                                  <div class="tab-pane active" id="tempalte">
                                    <div class="timeline-item">
                                        <iframe src="{{url('emails/preview/opencart')}}" style="width:100%; height: 700px">
                                        </iframe>
                                    </div>
                                  </div>
            ...
            <div class="box-body">
                ...
                <div class="table-responsive mailbox-messages">
                    <table id="opencarttable" class="table table-bordered table-striped">
                        ...
                        @if($opencarts)
                        <tbody>
                            @foreach ($opencarts as $design)
                                <tr>
                                    <td class="mailbox-name"><a href="{{route('designs.show', $design->id)}}"><input type="checkbox" class="opencart" id="{{$design->user->email}}"></a></td>
                                    <td class="mailbox-name"><a href="{{route('designs.show', $design->id)}}"><b>{{$design->user->firstname}} {{$design->user->lastname}}</b></a></td>
                                    <td class="mailbox-name"><a href="{{route('designs.show', $design->id)}}">{{$design->user->email}}</a></td>
                                    <td class="mailbox-name"><a href="{{route('designs.show', $design->id)}}">{{$design->updated_at->format('m/d/Y h:m:i')}}</a></td>
                                </tr>
                            @endforeach
                        </tbody>
                        @endif
