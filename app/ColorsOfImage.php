...
class ColorsOfImage {
	...
	var $_excluded_colors = array();
	var $dbcolors = array();

	static $hit_pixels = 0;
	static $missed_pixels = 0;

	public function __construct( $image, $precision = 1, $maxnumcolors = 9, $trueper = true, $dbcolors ) {
...
//exec('convert '.$this->workingImage." -fuzz 90% -transparent White ".$this->workingImage  , $result);
		...
		for( $x = 0; $x < $width; $x += $this->precision ) {
			for ( $y = 0; $y < $height; $y += $this->precision ) {
				...
				if ($index === false ||  $rgb["alpha"] == 127)
			    {
			        //echo "no color detected";
			    }
			    else
			    {  
			    	//match the detected colors with our pallete colors ink
			        //$color = $this->getClosestColor( $rgb["red"], $rgb["green"], $rgb["blue"] );
					//$hexarray[] = $this->RGBToHex( $color[0], $color[1], $color[2] );

					//detect the reals colors of the image
					$hexarray[] = $this->RGBToHex( $rgb["red"], $rgb["green"], $rgb["blue"] );
			    }  
	...
	public function getBackgroundColor( $use_palette = true ) 
	{
		...
		$top_left_index = imagecolorat($this->workingImage, 0, 0);
		$top_left_color = imagecolorsforindex( $this->workingImage, $top_left_index ); 
		if ($top_left_index === false ||  $top_left_color["alpha"] == 127)
	    {
	        //echo "no color detected";
	    }
	    else
	    ...
	    $top_right_index = imagecolorat( $this->workingImage, $this->width - 1, 0 ); 
		$top_right_color = imagecolorsforindex( $this->workingImage,$top_right_index );
		if ($top_right_index === false ||  $top_right_color["alpha"] == 127)
	    {
	        //echo "no color detected";
	    }
	    else
		...
		$bottom_left_index = imagecolorat( $this->workingImage, 0, $this->height - 1 );
		$bottom_left_color = imagecolorsforindex( $this->workingImage,  $bottom_left_index);
		if ($bottom_left_index === false ||  $bottom_left_color["alpha"] == 127)
	    {
	        //echo "no color detected";
	    }
	    else
		...
		$bottom_right_index = imagecolorat( $this->workingImage, $this->width - 1, $this->height - 1 );
		$bottom_right_color = imagecolorsforindex( $this->workingImage, $bottom_right_index );
		if ($bottom_right_index === false ||  $bottom_right_color["alpha"] == 127)
	    {
	        //echo "no color detected";
	    }
	    else
		...
		if ( $use_palette && $top_left != null  && $top_right != null  && $bottom_right != null  && $bottom_left != null ) {
			...
			if (count($colors) > 0 && $colors[0] != null)
			{
				if( count( array_unique( $colors[0] ) ) == 1 ) 
				{
					//echo $this->RGBToHex( $top_left[0], $top_left[1], $top_left[2] );
	...
	private function getPalette() 
	{
		...
		//$sql= 'SELECT * FROM `color` WHERE Type ="ink";';
        //$result = $this->db->query($sql);
        $str ='[';
        //$ListColors = $result->fetch_array(MYSQLI_ASSOC);
        foreach($this->dbcolors as $color) 
        {
        	 $str.= '"#'.$color.'",';
        }
        //$str= rtrim($str,',');
        //$str.=']';
        $str .= '"#660000", "#990000", "#e0feff", "#ea4c88", "#993399", "#663399", "#333399", "#0066cc", "#0099cc", "#66cccc", "#77cc33", "#669900", "#336600", "#666600", "#999900", "#cccc33", "#ffff00", "#ffcc33", "#ff9900", "#ff6600", "#cc6633", "#996633", "#663300", "#000000",  "#ffffff",  "#FDADC7", "#ABBCDA", "#F5DD01","#fed8c9"]';
